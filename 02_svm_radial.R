...
# .577 best
svm_radial_table <- svm_radial_tuned1 %>% 
  show_best()

save(svm_radial_table, file = "results/svm_radial_table.rda")
...
# Set up workflow
svm_radial_workflow2 <- workflow() %>% 
  add_model(svm_radial_model) %>% 
  add_recipe(recipe2)


# Tune model
svm_radial_tuned2 <- tune_grid(svm_radial_workflow2, 
                               folds,
                               grid = svm_radial_grid,
                               control = control_grid(save_pred = TRUE, # Create an extra column for each prediction
                                                      save_workflow = TRUE, # Lets you use extract_workflow
...
#save as rda
save(svm_radial_tuned2,
     file = "results/svm_radial_tuned2.rda")

load("results/svm_radial_tuned2.rda")
